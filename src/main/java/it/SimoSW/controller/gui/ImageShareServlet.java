...
public class ImageShareServlet extends HttpServlet {
    ...
    private static final Logger LOGGER = Logger.getLogger(ImageShareServlet.class.getName());
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        LOGGER.fine(() -> "[ImageShareServlet] Richiesta: engineRef=" + engineRef + ", imageIndex=" + indexStr);

        // Validazione parametri
        if (engineRef == null || engineRef.isBlank() || indexStr == null || indexStr.isBlank()) {
            LOGGER.warning("[ImageShareServlet] Parametri mancanti");
        ...
        if (filenameOpt.isEmpty()) {
            LOGGER.warning(() -> "[ImageShareServlet] Immagine non trovata per: " + engineRef + " index " + imageIndex);
            response.sendError(HttpServletResponse.SC_NOT_FOUND, "Immagine non trovata");
            return;
        }

        String filename = filenameOpt.get();
        LOGGER.fine(() -> "[ImageShareServlet] Filename trovato: " + filename);
        ...
        try {
            ...
            LOGGER.fine(() -> "[ImageShareServlet] Path assoluto: " + imagePath);
            LOGGER.fine(() -> "[ImageShareServlet] File esiste: " + imageFile.exists());

            if (!imageFile.exists() || !imageFile.isFile()) {
                LOGGER.warning(() -> "[ImageShareServlet] File non trovato: " + imagePath);
            ...
            LOGGER.log(Level.SEVERE, "[ImageShareServlet] Errore durante il caricamento", e);
    ...
    private void serveFile(HttpServletResponse response, File imageFile, String filename) throws IOException {
        ...
        try (FileInputStream fis = new FileInputStream(imageFile);
            ...
            os.flush();
            LOGGER.fine(() -> "[ImageShareServlet] File servito con successo: " + filename);
        }
    }
    ...
    private void serveFileViaResourceStream(HttpServletResponse response, String engineRef, String filename) throws IOException {
        ...
        LOGGER.fine(() -> "[ImageShareServlet] Usando getResourceAsStream: " + resourcePath);

        try (java.io.InputStream is = getServletContext().getResourceAsStream(resourcePath)) {
            if (is == null) {
                LOGGER.warning(() -> "[ImageShareServlet] Risorsa non trovata: " + resourcePath);
            ...
            try (OutputStream os = response.getOutputStream()) {
                ...
                os.flush();
                LOGGER.fine(() -> "[ImageShareServlet] File servito via ResourceStream: " + filename);
            }
        }
    }
}
